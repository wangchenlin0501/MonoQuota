...
详细面板在点击其他位置后自动收起。面板中可设置、取消或清除全局快捷键；设置后从其他应用按下该组合键，即可在菜单栏额度图标下方打开或收起面板。快捷键保存在本机，不需要辅助功能权限。

在 macOS 26 及更高版本上，详细卡片使用系统 Liquid Glass 效果；较早版本使用系统材质背景。

详细面板采用无尖角的圆角浮层，完整显示在菜单栏下方。进度条带柔和光晕、细高光和数值变化过渡；菜单栏数字区域按实际字体宽度布局，减少右侧多余留白。
